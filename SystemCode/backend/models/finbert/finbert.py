...
from pytorch_pretrained_bert.optimization import BertAdam, WarmupLinearSchedule as warmup_linear
...
# from models.finbert.utils import *
from models.finbert.utils import FinSentProcessor, convert_examples_to_features, chunks, InputExample, softmax
import numpy as np
import logging
...
def predict(text, model, write_to_csv=False, path=None):
    ...
    ----------
    text: string
        text to be analyzed (will be analysed per sentence)
    text: array of strings
        alternatively, text can be an array of strings. In this case, we will analyse each element (which can be multiple sentences)
    ...
    for batch in tqdm(chunks(sentences, 5), desc="Predicting", total=len(sentences)//5):
    ...
    if write_to_csv:
        if not os.path.exists(path):
            logger.warning("Unable to save file to csv at " + path)
        else:
